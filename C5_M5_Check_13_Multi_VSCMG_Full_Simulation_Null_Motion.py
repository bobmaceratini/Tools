...
w_BN_B_t0 = np.array([0.0, 0.0, 0.0]) # Initial angular velocity of Nano Spacecraft w.r.t. Inertial Frame expressed in Body Frame in rad/s    
...
s2 = 0.2
s3 = -0.3
sigma_ref[:,:] = 1/4*np.array([s1*np.sin(f*time), s2*np.cos(f*time), s3*np.sin(2*f*time)])*0
sigma_ref[0,:] = s_BN_t0[0]*np.ones((1,len(time)))
sigma_ref[1,:] = s_BN_t0[1]*np.ones((1,len(time)))
sigma_ref[2,:] = s_BN_t0[2]*np.ones((1,len(time)))
