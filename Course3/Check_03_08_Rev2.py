...
x = Kinetic_Integration(Quat_Differential, omega_b, (0,60), initial_quat, t_eval)
...
q42_bis = x[np.where(np.isclose(t_eval, 42.0))[0][0]]
print(f"Quaternion a t=42s (da integrazione Kinetic_Integration): {q42_bis}")
print(f"Norma del quaternion a t=42s (da integrazione Kinetic_Integration): {np.linalg.norm(q42_bis)}")
print(f"Valore iniziale del quaternion: {x[0]}")
print("Differenza tra i due quaternion a t=42s:", q42 - q42_bis)
