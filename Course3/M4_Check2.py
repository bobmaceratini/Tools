...
omega0_degs = np.array([30.0, 10.0, -20.0])
omega0 = omega0_degs/180*np.pi
...
A = np.block([[np.zeros((3,3)),np.eye(3)],[-K,-P]])
print(A)
#Simulation
t_eval = np.arange(0, 120.01, 0.01)
sigma_ref = np.array([s1*np.sin(f*t_eval), s2*np.cos(f*t_eval), s3*np.sin(f*t_eval)]).T*0
sigma_dot_ref = np.array([s1*f*np.cos(f*t_eval), -s2*f*np.sin(f*t_eval), s3*f*np.cos(f*t_eval)]).T*0
...
for i in range(1,len(t_eval)):
    ...
    x[i,:] = IntegratorRK4Step(x[i-1,:], A, np.zeros(3), np.zeros(3), dt)

s=x[:,0:3]
snorm = np.linalg.norm(s,axis=1)
# Plot results
plt.figure(figsize=(10, 6))
plt.plot(t_eval, x[:, 0],'--', label='Sigma 1')
plt.plot(t_eval, x[:, 1],'--', label='Sigma 2')
plt.plot(t_eval, x[:, 2],'--', label='Sigma 3')
plt.plot(t_eval, snorm, label='Sigma Norm', color='black', linewidth=2)
plt.title('Attitude Dynamics with State Feedback Control')
plt.xlabel('Time (s)')
...
print(f"Initial Omega [dag/s]: {omega0_degs}")
...
sigma,omega,angles,sigmaBR,u = Control_Linear_CLD_MPR(InertiaTensor,sigma0, omega0, K, P, 100000, t_eval, sigma_ref,omega_ref)
...
plt.figure(figsize=(10, 6))
plt.plot(t_eval, omega[:,0], label='sigma(1)', color='blue')
plt.plot(t_eval, omega_ref[:,0], '--', label='sigma\_ref(1)', color='blue')
plt.plot(t_eval, omega[:,1], label='sigma(2)', color='green')
plt.plot(t_eval, omega_ref[:,1], '--', label='sigma\_ref(2)', color='green')
plt.plot(t_eval, omega[:,2], label='sigma(3)', color='orange')
plt.plot(t_eval, omega_ref[:,2], '--', label='sigma\_ref(3)', color='orange')

plt.xlabel('Tempo [s]')
plt.ylabel('Angoli [rad/s]')
plt.title('Evoluzione velocità angolari')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.xlabel('Tempo [s]')
plt.ylabel('Angoli [rad/s]')
plt.title('Evoluzione differenze MRP')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.figure(figsize=(10, 6))
plt.plot(t_eval, u, label='u', color='blue')
plt.xlabel('Tempo [s]')
plt.ylabel('Angoli [rad/s]')
plt.title('Evoluzione differenze MRP')
plt.legend()
...
skip = 40  # Salta frame per velocizzare l'animazione
